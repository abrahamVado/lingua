/**
 * Behavior: principal executives modal
 * Depends: core/drupal, core/once
 */
(function (Drupal, once) {
  'use strict';

  Drupal.behaviors.pdsExecutives = {
    attach(context) {
      const grids = once('pds-executives-grid', '.principal-executives__grid', context);
      ...
      grids.forEach((grid) => {
        grid.addEventListener('click', function (e) {
          const btn = e.target.closest('.principal-executives__card-btn');
          if (!btn) return;
          lastFocused = btn;

          const card = btn.closest('.principal-executives__card');
          if (!card) return;

          const photo = card.getAttribute('data-photo') || '';
          const name  = card.getAttribute('data-name') || (card.querySelector('.principal-executives__name')?.textContent.trim() || '');
          const title = card.getAttribute('data-title') || '';
          const cvHTML= card.querySelector('.principal-executives__cv')?.innerHTML || '';
