/**
 * Behavior: principal fondos_mutuos modal
 * Depends: core/drupal, core/once
 */
(function (Drupal, once) {
  'use strict';

  Drupal.behaviors.pdsFondosMutuos = {
    attach(context) {
      const grids = once('pds-fondos_mutuos-grid', '.principal-fondos_mutuos__grid', context);
      ...
      grids.forEach((grid) => {
        grid.addEventListener('click', function (e) {
          const btn = e.target.closest('.principal-fondos_mutuos__card-btn');
          if (!btn) return;
          lastFocused = btn;

          const card = btn.closest('.principal-fondos_mutuos__card');
          if (!card) return;

          const photo = card.getAttribute('data-photo') || '';
          const name  = card.getAttribute('data-name') || (card.querySelector('.principal-fondos_mutuos__name')?.textContent.trim() || '');
          const title = card.getAttribute('data-title') || '';
          const cvHTML= card.querySelector('.principal-fondos_mutuos__cv')?.innerHTML || '';
