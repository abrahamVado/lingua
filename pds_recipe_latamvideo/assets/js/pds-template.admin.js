...
function clearInputs(root) {
  ...
  function updateTimelineDatasetSnapshot(root, rows) {
    ...
    if (typeof drupalSettings === 'undefined' || !drupalSettings || !drupalSettings.pdsRecipeLatamVideo) {
      return;
    }
    ...
    if (!drupalSettings.pdsRecipeLatamVideo.masters) {
      drupalSettings.pdsRecipeLatamVideo.masters = {};
    }
    if (!drupalSettings.pdsRecipeLatamVideo.masters[instanceUuid]) {
      drupalSettings.pdsRecipeLatamVideo.masters[instanceUuid] = { metadata: {}, datasets: {} };
    }

    var master = drupalSettings.pdsRecipeLatamVideo.masters[instanceUuid];
    if (!master.datasets) {
      master.datasets = {};
    }
