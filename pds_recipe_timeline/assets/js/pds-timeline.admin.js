(function (Drupal, once) {
  ...
  function sel(root, drupalSelector, idFallback) {
    ...
    //2.- Gracefully fall back to an id lookup for legacy markup.
    if (!el && idFallback) {
  ...
  function ensureErrorContainer(root) {
    ...
    container.setAttribute('data-pds-timeline-error', '1');
    container.className = 'pds-timeline-admin__errors';
  ...
  function readState(root) {
    ...
    var el = findField(root, {
      ds: 'pds-timeline-cards-state',
      id: 'pds-timeline-cards-state',
      nameEnd: '[cards_state]'
    });
    if (!el) {
  ...
  function readEditIndex(root) {
    ...
    var el = findField(root, {
      ds: 'pds-timeline-edit-index',
      id: 'pds-timeline-edit-index',
      nameEnd: '[edit_index]'
    });
    if (!el) {
  ...
  function applyGroupIdToDom(root, groupId) {
    ...
    root.setAttribute('data-pds-timeline-group-id', String(groupId));
    root._pdsTimelineGroupId = groupId;

    //2.- Mirror the value into the hidden field so Drupal receives it on submit.
    var hidden = findField(root, {
      ds: 'pds-timeline-group-id',
  ...
  function ensureGroupExists(root) {
    ...
    var url = root.getAttribute('data-pds-timeline-ensure-group-url');
    if (!url) {
      return Promise.resolve(null);
    }
    ...
    if (root._pdsGroupEnsured) {
      return Promise.resolve(root._pdsTimelineGroupId || null);
    }
  ...
  function getImageFid(root) {
    ...
    var wrapper = root.querySelector('#pds-timeline-image,[id^="pds-timeline-image"],[data-drupal-selector$="image"]');
    if (!wrapper) {
      return null;
    }
...
function clearInputs(root) {
  ...
  function resolveRowViaAjax(root, row) {
    ...
    return new Promise(function (resolve) {
      ...
      var url = root.getAttribute('data-pds-timeline-resolve-row-url');
      if (!url) {
  ...
  function createRowViaAjax(root, row) {
    ...
    var url = root.getAttribute('data-pds-timeline-create-row-url');
    if (!url) {
    ...
    var recipeType = root.getAttribute('data-pds-timeline-recipe-type');
    if (recipeType) {
  ...
  function updateRowViaAjax(root, row, idx) {
    ...
    var template = root.getAttribute('data-pds-timeline-update-row-url');
    if (!template) {
  ...
  function getPreviewWrapper(root) {
    ...
    var wrapper = root.querySelector('[data-pds-timeline-preview-root]');
    if (wrapper) {
      return wrapper;
    }

    //2.- Fall back to the legacy container so older markup keeps working.
    return root.querySelector('#pds-timeline-preview-list');
  }

  function setPreviewState(wrapper, state, message) {
    //1.- Only toggle states when the upgraded markup is present.
    if (!wrapper || !wrapper.hasAttribute('data-pds-timeline-preview-root')) {
      return;
    }

    var states = ['loading', 'empty', 'error', 'content'];
    states.forEach(function (name) {
      var region = wrapper.querySelector('[data-pds-timeline-preview-state="' + name + '"]');
      if (!region) {
        return;
      }
  ...
  function renderPreviewTable(root) {
    ...
    var hasStates = wrapper.hasAttribute('data-pds-timeline-preview-root');
    var content = hasStates
      ? wrapper.querySelector('[data-pds-timeline-preview-state="content"]')
      : wrapper;
    if (!content) {
      return;
    }
    ...
    if (!rows.length) {
      ...
      if (!root._pdsPreviewAutoLoaded) {
        var listingUrl = root.getAttribute('data-pds-timeline-list-rows-url');
        if (listingUrl) {
      ...
      if (hasStates) {
        ...
        content.innerHTML =
          '<table class="pds-timeline-table">' +
            ...
            '<tbody>' +
              '<tr>' +
                '<td colspan="6"><em>No rows yet.</em></td>' +
              '</tr>' +
            '</tbody>' +
          '</table>';
      }
    ...
    var html = '';
    html += '<table class="pds-timeline-table">';
    html +=   '<thead>';
    html +=     '<tr>';
    html +=       '<th>ID</th>';
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var rawJson = typeof r.description_json === 'string' ? r.description_json : '';
      var jsonPreview = '—';
      if (rawJson.trim() !== '') {
        var shortened = rawJson;
        if (shortened.length > 120) {
          shortened = shortened.slice(0, 120) + '…';
        }
        jsonPreview = shortened;
      }

      html +=   '<td class="pds-timeline-table__json">' + escapeHtml(jsonPreview) + '</td>';
  ...
  function normalizeTimelineEntries(source) {
    //1.- Decode string sources before evaluating the payload type.
    ...
    if (Array.isArray(source)) {
      //2.- Accept direct arrays as-is for backwards compatibility with legacy snapshots.
      candidateList = source;
    }
    else if (source && typeof source === 'object') {
      if (Array.isArray(source.segments)) {
        candidateList = source.segments;
      }
      else if (Array.isArray(source.timeline)) {
        candidateList = source.timeline;
      }
      else {
        candidateList = Object.keys(source).map(function (key) {
          var value = source[key];
          if (value && typeof value === 'object') {
            return value;
          }
          return { label: key, info: value };
        });
      }
    }
    ...
    var normalized = [];
    candidateList.forEach(function (entry, position) {
      var segment = coerceTimelineSegment(entry, position);
      if (segment) {
  ...
  function coerceTimelineSegment(entry, position) {
    //1.- Allow terse primitives by expanding them into objects with a label or width.
    ...
    if (label === '' && typeof entry.key === 'string') {
      label = entry.key.trim();
    }

    var infoText = '';
    ['info', 'tooltip', 'description', 'text', 'label'].some(function (key) {
      if (typeof entry[key] === 'string' && entry[key].trim() !== '') {
        infoText = entry[key].trim();
        return true;
      }
      return false;
    });
    ...
    var widthStyle = formatWidthStyle(entry.width_percent || entry.width || entry.percentage || entry.span);
    var infoImage = '';
    ['image', 'image_url', 'logo', 'img'].some(function (key) {
      if (typeof entry[key] === 'string' && entry[key].trim() !== '') {
        infoImage = entry[key].trim();
        return true;
      }
      return false;
    });
    ...
    var infoImageAlt = '';
    ['image_alt', 'alt'].some(function (key) {
      if (typeof entry[key] === 'string' && entry[key].trim() !== '') {
        infoImageAlt = entry[key].trim();
        return true;
      }
      return false;
    });
    if (infoImageAlt === '' && label !== '') {
      infoImageAlt = label;
    }

    var isPrincipal = false;
    ['principal', 'is_principal'].some(function (key) {
      if (typeof entry[key] !== 'undefined') {
        isPrincipal = Boolean(entry[key]);
        return true;
      }
      return false;
    });
    if (!isPrincipal && typeof entry.type === 'string') {
      isPrincipal = entry.type.toLowerCase() === 'principal';
    }
    if (!isPrincipal && typeof entry.variant === 'string') {
      isPrincipal = entry.variant.toLowerCase() === 'principal';
    }

    var isFirst = false;
    ['first', 'is_first', 'placeholder'].some(function (key) {
      if (typeof entry[key] !== 'undefined') {
        isFirst = Boolean(entry[key]);
        return true;
      }
      return false;
    });
    ...
    if (Array.isArray(entry.classes)) {
      classes = entry.classes.filter(function (value) {
        return typeof value === 'string' && value.trim() !== '';
      }).map(function (value) {
        return value.trim();
      });
    }
    else if (typeof entry.classes === 'string' && entry.classes.trim() !== '') {
      classes = entry.classes.trim().split(/\s+/);
    }

    var tooltipVariant = '';
    if (typeof entry.tooltip_variant === 'string') {
      tooltipVariant = entry.tooltip_variant.trim();
    } else if (isPrincipal) {
    ...
    return {
      ...
      info_image_alt: infoImageAlt || label,
  ...
  function formatWidthStyle(value) {
    //1.- Accept CSS-ready percentages immediately.
  ...
  function updateTimelineDatasetSnapshot(root, rows) {
    ...
    if (typeof drupalSettings === 'undefined' || !drupalSettings || !drupalSettings.pdsRecipeTimeline) {
      return;
    }

    var instanceUuid = root.getAttribute('data-pds-timeline-block-uuid');
    if (!instanceUuid) {
      return;
    }

    if (!drupalSettings.pdsRecipeTimeline.masters) {
      drupalSettings.pdsRecipeTimeline.masters = {};
    }
    if (!drupalSettings.pdsRecipeTimeline.masters[instanceUuid]) {
      drupalSettings.pdsRecipeTimeline.masters[instanceUuid] = { metadata: {}, datasets: {} };
    }

    var master = drupalSettings.pdsRecipeTimeline.masters[instanceUuid];
    if (!master.datasets) {
      master.datasets = {};
    }
    ...
    rows.forEach(function (row) {
      ...
      master.datasets.timeline[key] = {
        ...
        description_json: typeof row.description_json === 'string' ? row.description_json : ''
      };
    });
  }
  ...
  function refreshPreviewFromServer(root) {
    ...
    var url = root.getAttribute('data-pds-timeline-list-rows-url');
    if (!url) {
      renderPreviewTable(root);
      return Promise.resolve(null);
    }
      ...
      .then(function (json) {
        ...
        var normalized = json.rows.map(function (row) {
          ...
          var normalizedRow = {
            ...
            description_json: typeof safeRow.description_json === 'string' ? safeRow.description_json : '',
          ...
          var normalizedTimeline = normalizeTimelineEntries(safeRow.description_json || safeRow.timeline);
  ...
  function bindRowButtons(root, wrapper) {
    ...
    selAll(wrapper, '.pds-timeline-row-edit').forEach(function (btn) {
    ...
    selAll(wrapper, '.pds-timeline-row-del').forEach(function (btn) {
      btn.addEventListener('click', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var idx = parseInt(this.closest('tr').getAttribute('data-row-index'), 10);
  ...
  function startEditRow(root, idx) {
    ...
    var btn = sel(root, 'pds-timeline-add-card', 'pds-timeline-add-card');
    if (btn) {
...
function initFileWidgetTimeline(root) {
...
function rebuildManagedFileEmpty(root) {
  ...
  function activateTab(root, panelId) {
    ...
    selAll(root, '[data-pds-timeline-tab-target]').forEach(function (btn) {
      var match = (btn.getAttribute('data-pds-timeline-tab-target') === panelId);
      if (match) {
    ...
    selAll(root, '.js-pds-timeline-panel').forEach(function (panel) {
      var match = (panel.getAttribute('data-pds-timeline-panel-id') === panelId);
      if (match) {
  ...
  function initTabs(root) {
    ...
    selAll(root, '[data-pds-timeline-tab-target]').forEach(function (btn) {
      btn.addEventListener('click', function (event) {
        ...
        var targetId = btn.getAttribute('data-pds-timeline-tab-target');
        activateTab(root, targetId);
      });
    });

    //2.- Default to the first tab to keep the UX predictable on initial load.
    var firstBtn = root.querySelector('[data-pds-timeline-tab-target]');
    if (firstBtn) {
      activateTab(root, firstBtn.getAttribute('data-pds-timeline-tab-target'));
    }
  }
  ...
  Drupal.behaviors.pdsTimelineAdmin = {
    attach: function (context) {
      once('pdsTimelineAdminRoot', '.js-pds-timeline-admin', context).forEach(function (root) {
        ...
        if (!hasInitialRows && root.getAttribute('data-pds-timeline-list-rows-url')) {
        ...
        initFileWidgetTimeline(root);

        //3.- Cache any precomputed group id exposed by PHP so fetch handler can reuse it.
        var existingGroupIdAttr = root.getAttribute('data-pds-timeline-group-id');
        if (existingGroupIdAttr) {
          var parsedExistingId = parseInt(existingGroupIdAttr, 10);
          if (!isNaN(parsedExistingId)) {
            root._pdsTimelineGroupId = parsedExistingId;
          }
        }

        // Add row / Save changes button.
        // Accept <a ... data-drupal-selector="pds-timeline-add-card"> or <div ... same attrs>.
        var addBtn =
          root.querySelector('[data-drupal-selector="pds-timeline-add-card"], #pds-timeline-add-card');
        ...
        if (hostForm) {
          function cacheSubmitter(target) {
            ...
            if (submitter) {
              ...
              root._pdsTimelineSubmitter = submitter;
            }
          }
          ...
          hostForm.addEventListener('submit', function (event) {
            ...
            var submitter = event.submitter || root._pdsTimelineSubmitter || null;
            if (!submitter) {
