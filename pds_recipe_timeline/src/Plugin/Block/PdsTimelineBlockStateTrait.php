 ...
 */
trait PdsTimelineBlockStateTrait {
  ...
  private function setGroupIdOnFormState(FormStateInterface $form_state, int $group_id): void {
    ...
    $form_state->set('pds_timeline_group_id', $group_id);
    //2.- Mirror into the temporary store for subform rebuilds triggered by AJAX.
    $form_state->setTemporaryValue('pds_timeline_group_id', $group_id);

    if ($form_state instanceof SubformStateInterface && method_exists($form_state, 'getCompleteFormState')) {
      $parent = $form_state->getCompleteFormState();
      if ($parent instanceof FormStateInterface) {
        //3.- Bubble the id up so parent form handlers can also reach it.
        $parent->set('pds_timeline_group_id', $group_id);
        $parent->setTemporaryValue('pds_timeline_group_id', $group_id);
      }
    }
  }
  ...
  private function getGroupIdFromFormState(FormStateInterface $form_state): ?int {
    ...
    if ($form_state->has('pds_timeline_group_id')) {
      $value = $form_state->get('pds_timeline_group_id');
      if (is_numeric($value)) {
        return (int) $value;
      }
    }
    ...
    if ($form_state->hasTemporaryValue('pds_timeline_group_id')) {
      $value = $form_state->getTemporaryValue('pds_timeline_group_id');
      if (is_numeric($value)) {
        return (int) $value;
      }
    }
