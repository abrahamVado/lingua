...
trait PdsTimelineRenderContextTrait {
  ...
  private function buildExtendedDatasets(array $items): array {
    ...
    foreach ($items as $delta => $row) {
      ...
      $segments = [];
      if (isset($row['timeline_segments']) && is_array($row['timeline_segments'])) {
        $segments = $row['timeline_segments'];
      }
      if ($segments !== []) {
        $timeline_rows[] = [
          'index' => $delta,
      ...
      if (isset($row['timeline_meta']) && is_array($row['timeline_meta'])) {
        $meta = $row['timeline_meta'];
        if ($timeline_heading === '' && isset($meta['heading']) && is_string($meta['heading'])) {
          $timeline_heading = trim($meta['heading']);
        }
        if ($timeline_years === [] && isset($meta['years']) && is_array($meta['years'])) {
          $timeline_years = array_values(array_filter(array_map(static function ($value) {
    ...
    if ($timeline_heading === '') {
      $timeline_heading = (string) $this->t('Principais executivos');
    }
