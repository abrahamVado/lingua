...
/**
 * Provides helper methods to ensure timeline groups exist.
 */
final class GroupEnsurer {
   ...
   */
  public function ensure(string $uuid, string $type = 'pds_recipe_timeline'): ?int {
   ...
   */
  public function ensureGroupAndGetId(string $uuid, string $type = 'pds_recipe_timeline'): ?int {
    //1.- Delegate to the procedural helper so the authoritative logic lives in the .module file.
    $resolved_id = \pds_recipe_timeline_ensure_group_and_get_id($uuid, $type);
