...
final class PdsTemplateBlock extends BlockBase {
  ...
  public function defaultConfiguration(): array {
    return [
      ...
      'recipe_type' => 'pds_recipe_avisos_normativos',
    ];
  }
  ...
  private function ensureGroupAndGetId(): int {
    ...
    $type = $this->configuration['recipe_type'] ?? 'pds_recipe_avisos_normativos';

    //1.- Leverage the shared procedural helper so controllers and forms reuse identical logic.
    $group_id = \pds_recipe_avisos_normativos_ensure_group_and_get_id($uuid, $type);
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $recipe_type = $this->configuration['recipe_type'] ?? 'pds_recipe_avisos_normativos';
    ...
    $ensure_group_url = Url::fromRoute(
      'pds_recipe_avisos_normativos.ensure_group',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
    ...
    $resolve_row_url = Url::fromRoute(
      'pds_recipe_avisos_normativos.resolve_row',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
      ],
    ...
    $create_row_url = Url::fromRoute(
      'pds_recipe_avisos_normativos.create_row',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
        'query' => ['type' => $recipe_type],
    ...
    $update_row_url = Url::fromRoute(
      'pds_recipe_avisos_normativos.update_row',
      [
        'uuid' => $block_uuid,
    ...
    $list_rows_url = Url::fromRoute(
      'pds_recipe_avisos_normativos.list_rows',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
        'query' => $list_query,
    ...
    $form['pds_template_admin'] = [
      ...
      '#attached' => [
        'library' => [
          'pds_recipe_avisos_normativos/pds_template.admin',
        ],
      ],
    ];
  ...
  public static function ajaxResolveRow(Request $request, string $uuid): JsonResponse {
    ...
    if (!pds_recipe_avisos_normativos_user_can_manage_template()) {
    ...
    $promoter = \pds_recipe_avisos_normativos_resolve_row_image_promoter();
    if (!$promoter) {
      return new JsonResponse([
        'status' => 'error',
