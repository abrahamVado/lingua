...
namespace Drupal\pds_recipe_avisos_normativos\Controller;
...
final class RowController extends ControllerBase {
  ...
  public function createRow(Request $request, string $uuid): JsonResponse {
    ...
    if (!is_string($type) || $type === '') {
      if (isset($payload['recipe_type']) && is_string($payload['recipe_type']) && $payload['recipe_type'] !== '') {
        ...
        $type = 'pds_recipe_avisos_normativos';
      }
    }
    ...
    try {
      ...
      \Drupal::logger('pds_recipe_avisos_normativos')->error('Row creation failed for group @group: @message', [
        '@group' => $uuid,
  ...
  private function resolveSchemaRepairer(): ?object {
    ...
    return \pds_recipe_avisos_normativos_resolve_schema_repairer();
  }
