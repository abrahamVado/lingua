 ...
 *   admin_label = @Translation("PDS Slider Banner"),
 *   category = @Translation("PDS Recipes")
 * )
 */
final class PdsSliderBannerBlock extends BlockBase {
  use PdsSliderBannerBlockStateTrait;
  use PdsSliderBannerRenderContextTrait;
  ...
  public function defaultConfiguration(): array {
    return [
      ...
      'recipe_type' => 'pds_recipe_slider_banner',
    ];
  }
  ...
  private function ensureGroupAndGetId(): int {
    ...
    $type = $this->configuration['recipe_type'] ?? 'pds_recipe_slider_banner';
  ...
  public function blockForm($form, FormStateInterface $form_state) {
    ...
    $recipe_type = $this->configuration['recipe_type'] ?? 'pds_recipe_slider_banner';
    ...
    $ensure_group_url = Url::fromRoute(
      'pds_recipe_slider_banner.ensure_group',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
    ...
    $resolve_row_url = Url::fromRoute(
      'pds_recipe_slider_banner.resolve_row',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
      ],
    ...
    $create_row_url = Url::fromRoute(
      'pds_recipe_slider_banner.create_row',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
        'query' => ['type' => $recipe_type],
    ...
    $update_row_url = Url::fromRoute(
      'pds_recipe_slider_banner.update_row',
      [
        'uuid' => $block_uuid,
    ...
    $list_rows_url = Url::fromRoute(
      'pds_recipe_slider_banner.list_rows',
      ['uuid' => $block_uuid],
      [
        'absolute' => TRUE,
        'query' => $list_query,
    ...
    $form['pds_slider_banner_admin'] = [
      '#type' => 'container',
      //1.- Enable tree handling so blockSubmit() can retrieve nested values via parent keys.
      '#tree' => TRUE,
      '#attributes' => [
        'class' => [
          'pds-slider-banner-admin',
          'js-pds-slider-banner-admin',
        ],
        //5.- Expose AJAX endpoint and identifiers so JS can ensure persistence on add.
        'data-pds-slider-banner-ensure-group-url' => $ensure_group_url,
        'data-pds-slider-banner-resolve-row-url' => $resolve_row_url,
        'data-pds-slider-banner-create-row-url' => $create_row_url,
        'data-pds-slider-banner-update-row-url' => $update_row_url,
        'data-pds-slider-banner-list-rows-url' => $list_rows_url,
        'data-pds-slider-banner-group-id' => (string) $group_id,
        'data-pds-slider-banner-block-uuid' => $block_uuid,
        'data-pds-slider-banner-recipe-type' => $recipe_type,
      ],
      '#attached' => [
        'library' => [
          'pds_recipe_slider_banner/pds_recipe_slider_banner.admin',
        ],
      ],
    ];

    $form['pds_slider_banner_admin']['group_id'] = [
      '#type' => 'hidden',
      '#attributes' => [
        'data-drupal-selector' => 'pds-slider-banner-group-id',
    ...
    $form['pds_slider_banner_admin']['edit_index'] = [
      '#type' => 'hidden',
      '#attributes' => [
        'data-drupal-selector' => 'pds-slider-banner-edit-index',
      ],
    ...
    $form['pds_slider_banner_admin']['tabs_nav'] = [
      '#type' => 'container',
      '#attributes' => [
        'class' => [
          'pds-slider-banner-admin__tabs-horizontal',
        ],
        'data-pds-slider-banner-tabs' => 'nav',
      ],
    ];

    //8.- Keep these elements as buttons even though Drupal renders submit inputs because JS cancels submission.
    $form['pds_slider_banner_admin']['tabs_nav']['tab_a'] = [
      '#type' => 'button',
      '#value' => $this->t('Tab A'),
      '#attributes' => [
        'class' => [
          'pds-slider-banner-admin__tab-btn-horizontal',
        ],
        'data-pds-slider-banner-tab-target' => 'panel-a',
      ],
    ];

    $form['pds_slider_banner_admin']['tabs_nav']['tab_b'] = [
      '#type' => 'button',
      '#value' => $this->t('Tab B'),
      '#attributes' => [
        'class' => [
        ...
        'data-pds-slider-banner-tab-target' => 'panel-b',
      ],
    ];

    //9.- Create the panels wrapper that houses the editable form and preview tabs.
    $form['pds_slider_banner_admin']['tabs_panels_wrapper'] = [
      '#type' => 'container',
      '#attributes' => [
        'id' => 'pds-slider-banner-panels-wrapper',
      ],
    ];

    $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels'] = [
      '#type' => 'container',
      '#attributes' => [
        'class' => [
          'pds-slider-banner-admin__panels-horizontal',
        ],
        'data-pds-slider-banner-tabs' => 'panels',
      ],
    ];

    //10.- Provide the edit panel container where the form widgets live.
    $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a'] = [
      '#type' => 'container',
      '#attributes' => [
        'id' => 'pds-slider-banner-panel-a',
        'class' => [
          'pds-slider-banner-admin__panel-horizontal',
          'js-pds-slider-banner-panel',
        ],
        'data-pds-slider-banner-panel-id' => 'panel-a',
      ],
    ];

  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a']['header'] = [
    '#type' => 'textfield',
    '#title' => $this->t('Header'),
    '#maxlength' => 255,
    '#attributes' => [
      'data-drupal-selector' => 'pds-slider-banner-header',
      'id' => 'pds-slider-banner-header',
    ],
  ];

  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a']['subheader'] = [
    '#type' => 'textfield',
    '#title' => $this->t('Subheader'),
    '#maxlength' => 255,
    '#attributes' => [
      'data-drupal-selector' => 'pds-slider-banner-subheader',
      'id' => 'pds-slider-banner-subheader',
    ],
  ];

  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a']['description'] = [
    '#type' => 'textarea',
    '#title' => $this->t('Description'),
    '#rows' => 3,
    '#attributes' => [
      'data-drupal-selector' => 'pds-slider-banner-description',
      'id' => 'pds-slider-banner-description',
    ],
  ];
  ...
  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a']['link'] = [
    '#type' => 'url',
    '#title' => $this->t('Link'),
    '#maxlength' => 512,
    '#attributes' => [
      'data-drupal-selector' => 'pds-slider-banner-link',
      'id' => 'pds-slider-banner-link',
    ],
  ];

  //11.- Render the add row trigger that allows editors to append new cards via JS.
  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_a']['add_item'] = [
    '#type' => 'markup',
    ...
    '#markup' => '<div class="pds-slider-banner-admin__add-btn" data-drupal-selector="pds-slider-banner-add-card" id="pds-slider-banner-add-card" role="button" tabindex="0">Add row</div>',
  ];


  //14.- Define the preview panel container that receives hydrated markup from AJAX calls.
  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_b'] = [
    '#type' => 'container',
    '#attributes' => [
      'id' => 'pds-slider-banner-panel-b',
      'class' => [
        'pds-slider-banner-admin__panel-horizontal',
        'js-pds-slider-banner-panel',
        'is-active',
      ],
      'data-pds-slider-banner-panel-id' => 'panel-b',
    ],
  ];
  ...
  $form['pds_slider_banner_admin']['tabs_panels_wrapper']['tabs_panels']['panel_b']['preview_list'] = [
    '#type' => 'markup',
    '#markup' => '<div id="pds-slider-banner-preview-list" class="pds-slider-banner-preview" data-pds-slider-banner-preview-root="1">'
      . '<div class="pds-slider-banner-preview__loading" data-pds-slider-banner-preview-state="loading" hidden>' . $loading_text . '</div>'
      . '<div class="pds-slider-banner-preview__empty" data-pds-slider-banner-preview-state="empty" hidden>' . $empty_text . '</div>'
      . '<div class="pds-slider-banner-preview__error" data-pds-slider-banner-preview-state="error" hidden>' . $error_text . '</div>'
      . '<div class="pds-slider-banner-preview__content" data-pds-slider-banner-preview-state="content"></div>'
      . '</div>',
  ];
  ...
  private function buildCardsMarkup(array $items): string {
    ...
    $table_head =
      '<thead>' .
        '<tr>' .
          '<th class="pds-slider-banner-table__col-thumb">' . $this->t('Image') . '</th>' .
          '<th>' . $this->t('Header') . '</th>' .
    ...
    if ($items === [] || count($items) === 0) {
      ...
      return '<table class="pds-slider-banner-table">' . $table_head . $table_body . '</table>';
    }
    ...
    foreach ($items as $delta => $item) {
      ...
      $rows_markup .=   '<td class="pds-slider-banner-table__thumb">' . $thumb_markup . '</td>';
      $rows_markup .=   '<td>' . $header . '</td>';
      $rows_markup .=   '<td>' . $subheader . '</td>';
      $rows_markup .=   '<td>' . $link . '</td>';
      $rows_markup .=   '<td>';
      $rows_markup .=     '<button type="button" class="pds-slider-banner-row-edit">' . $this->t('Edit') . '</button> ';
      $rows_markup .=     '<button type="button" class="pds-slider-banner-row-del">' . $this->t('Delete') . '</button>';
      $rows_markup .=   '</td>';
      $rows_markup .= '</tr>';
    }

    //4.- Return the assembled table HTML ready for the AJAX container.
    return '<table class="pds-slider-banner-table">' .
      $table_head .
  ...
  public function ajaxItems(array &$form, FormStateInterface $form_state) {
    ...
    return [
      '#type' => 'container',
      '#attributes' => [
        'id' => 'pds-slider-banner-preview-list',
      ],
      'inner' => [
        '#type' => 'item',
        '#markup' => $cards_markup,
