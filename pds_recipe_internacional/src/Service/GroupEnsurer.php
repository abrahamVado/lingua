...
namespace Drupal\pds_recipe_internacional\Service;
...
final class GroupEnsurer {
   ...
   */
  public function ensure(string $uuid, string $type = 'pds_recipe_internacional'): ?int {
   ...
   */
  public function ensureGroupAndGetId(string $uuid, string $type = 'pds_recipe_internacional'): ?int {
    //1.- Delegate to the procedural helper so the authoritative logic lives in the .module file.
    $resolved_id = \pds_recipe_internacional_ensure_group_and_get_id($uuid, $type);
